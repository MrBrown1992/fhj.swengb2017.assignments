...
object Graph {
  ...
  def mkGraph(start: Pt2D,
    ...
    def createTree(currentTree: Tree[L2D],
                   depth: Int,
      ...
      if (depth == maxDepth)
        ...
        def addNewRecursion(tree: Tree[L2D], curentRecursion: Int): Branch[L2D] = {
          tree match {
            case Node(root) =>
              createSubTree(Node(root), factor, angle, colorMap(curentRecursion))
            case Branch(Node(root), Branch(Node(left), Node(right))) =>

              val newSubtreeLeft =
                createSubTree(Node(left), factor, angle, colorMap(curentRecursion))
              val newSubtreeRight =
                createSubTree(Node(right), factor, angle, colorMap(curentRecursion))
              Branch(Node(root), Branch(newSubtreeLeft, newSubtreeRight))
            case Branch(Node(root), Branch(left, right)) =>
              Branch(Node(root),
                Branch(addNewRecursion(left, depth + 1),
                  addNewRecursion(right, depth + 1)))

          }
        }

        createTree(addNewRecursion(currentTree, depth), depth + 1, maxDepth)
      }
    }
